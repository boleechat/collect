...
class Spider(Spider):
    ...
    # API URL templates - Modified to include offset parameter
    api_url_template = "https://pc.api.btime.com/btimeweb/infoFlow?list_id=btv_08da67cea600bf3c78973427bfaba12d_s0_{year}_{month:02d}&refresh=1&count=31&page={page}"
    ...
    # Maximum number of pages to fetch per month
    max_pages_per_month = 10  # Increased from 1 to 10
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # Fetch videos for the selected year - removed limit to get all videos
    ...
    def fetchDataForYear(self, year):
        ...
        for month in range(months_to_fetch, 0, -1):
            # Fetch multiple pages for each month
            for page in range(1, self.max_pages_per_month + 1):
                api_url = self.api_url_template.format(year=year, month=month, page=page)
                ...
                try:
                    ...
                    if "data" in json_data and "list" in json_data["data"]:
                        ...
                        # If no items returned, we've hit the end of available data for this month
                        if not items:
                            break
                        ...
                        # No more data available for this month
                        break
                            
                except Exception as e:
                    print(f"Error fetching data for {year}-{month} page {page}: {e}")
                    continue
        
        # Cache the results
        self.data_cache[year] = data
        print(f"Fetched {len(data)} videos for year {year}")
        return data
